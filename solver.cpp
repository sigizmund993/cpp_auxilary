...
{
    Point r1 = (v_m + v_start) * (v_m - v_start).mag() / (2 * a_max);
    Point r3 = (v_m + v_end) * (v_m - v_end).mag() / (2 * a_max);
    Point r2 = r_end - r_start - r1 - r3;
    double t1 = (v_m - v_start).mag() / a_max;
    double t2 = r2.mag() / v_max;
    double t3 = (v_end - v_m).mag() / a_max;
    Point a,v,r;
    if (t < 0)
    {
        a = Point(0, 0);
    ...
    {
        a = (v_m - v_start).unity() * a_max;
        v = v_start + a * t;
        r = r_start + (v + v_start) * t / 2;
    }
    ...
    else if (t < t1 + t2)
    {
        a = Point(0, 0);
        v = v_m;
        r = r_start + r1 + v_m * (t - t1);
    }
    else if (t < t1 + t2 + t3)
    {
        a = (v_end - v_m).unity() * a_max;
        v = v_m + a * (t - t1 - t2);
        r = r_start + r1 + r2 + (v + v_m) * (t - t1 - t2) / 2;
    }
    else
    {
        a = Point(0, 0);
...
int main(void) {
    float MAX_SPEED = 2500;
    float MAX_ACC = 2000;
    Point start_pos = Point(0,0);
    Point start_vel = Point(1000,-1000);
    Point tgt_r = Point(2000,2000);
    Point tgt_v = Point(-1000,0);
    vector<double> v_m_vec = bangbang(vector<double>(start_vel.x,start_vel.y),vector<double>(tgt_v.x,tgt_v.y),vector<double>(tgt_r.x-start_pos.x,tgt_r.y-start_pos.y),MAX_ACC,MAX_SPEED);
    Point v_m = Point(v_m_vec[0],v_m_vec[1]);
    Point a,v,r;
    float t;
    get_bb_vals(0,start_pos,start_vel,tgt_r,tgt_v,MAX_ACC,MAX_SPEED,v_m,&a,&v,&r,&t);
    cout << a.x <<" " << a.y << t << endl;
    cout << "ZOV" << endl;
